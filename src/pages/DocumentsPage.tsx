...
function FolderTree({
  ...
  return <div className="space-y-0.5">{roots.map(renderFolder)}</div>;
}
    ...
    >
      ...
      >
        ...
        <div className="px-5 py-4 flex flex-wrap gap-x-6 gap-y-2 text-xs text-gray-500 border-t border-gray-100">
          <span>
            <span className="text-gray-400">Uploaded by</span> {file.uploadedBy}
          </span>
          <span>
            <span className="text-gray-400">Size</span> {file.size}
          </span>
          <span>
            <span className="text-gray-400">Usage</span> {file.usageInfo}
          </span>
          {file.inherited && (
...
export default function DocumentsPage() {
  ...
  const filtered = documents.filter((doc) => {
    if (selectedFolder !== "all") {
      const childIds = folders
        .filter((f) => f.parentId === selectedFolder)
        .map((f) => f.id);
  ...
  const folderLabel =
    folders.find((f) => f.id === selectedFolder)?.label ?? "All Files";

  return (
    <PageWrapper>
    <div className="flex min-h-[calc(100vh-170px)] overflow-hidden rounded-xl border border-gray-100 bg-white shadow-sm">
      {/* ── Left: folder tree ── */}
