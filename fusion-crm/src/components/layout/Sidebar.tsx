...
function NavItemRow({ item, collapsed }: { item: NavItem; collapsed: boolean }) {
  ...
  if (item.children) {
    return (
      <div>
        ...
        >
          <Icon className={clsx('w-4 h-4 shrink-0', isChildActive ? 'text-orange-500' : 'text-gray-400 group-hover:text-gray-600')} />
          {!collapsed && (
            <>
  ...
  return (
    ...
    >
      {({ isActive }) => (
        <>
          <Icon className={clsx('w-4 h-4 shrink-0', isActive ? 'text-orange-500' : 'text-gray-400 group-hover:text-gray-600')} />
      ...
      <div className="h-14 flex items-center justify-between px-3 border-b border-gray-100 shrink-0">
        {!collapsed && (
          <>
            ...
            <button onClick={onToggle} className="p-1 rounded-md hover:bg-gray-100 text-gray-400 shrink-0">
              <ChevronRight className="w-3.5 h-3.5 rotate-180" />
            </button>
          </>
        )}
        {collapsed && (
          <img src={logoFull} alt="Fusion CRM" className="h-7 w-7 object-contain mx-auto" />
        )}
      </div>
