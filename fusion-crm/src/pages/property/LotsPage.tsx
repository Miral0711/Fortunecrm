...
const LOTS: Lot[] = [
  { id:'1',  name:'Harlow Estate - 741',   suburb:'Tarneit',      state:'VIC', price:878028, type:'House and Land', status:'Available', rentalYield:'3.43%', tags:['HIGH CAPITAL GROWTH','SMSF'], date:'17 Apr 2026', imgGrad:'from-slate-300 to-slate-400',   lat:-37.84, lng:144.65 },
  { id:'2',  name:'Hamlet Estate - 39',    suburb:'Diggers Rest', state:'VIC', price:871477, type:'House and Land', status:'Available', rentalYield:'3.58%', tags:['HIGH CAPITAL GROWTH','SMSF'], date:'17 Apr 2026', imgGrad:'from-stone-300 to-stone-400',   lat:-37.60, lng:144.71 },
  { id:'3',  name:'Kinship - 135',         suburb:'Tarneit',      state:'VIC', price:867145, type:'House and Land', status:'Available', rentalYield:'3.21%', tags:['HIGH CAPITAL GROWTH'],        date:'16 Apr 2026', imgGrad:'from-amber-200 to-amber-300',   lat:-38.10, lng:145.33 },
  { id:'4',  name:'Horizon Estate - 22',   suburb:'Werribee',     state:'VIC', price:720000, type:'House and Land', status:'Available', rentalYield:'3.75%', tags:['SMSF'],                       date:'15 Apr 2026', imgGrad:'from-sky-200 to-sky-300',       lat:-37.90, lng:144.66 },
  { id:'5',  name:'Parkview - 26',         suburb:'Penrith',      state:'NSW', price:810000, type:'Duplex',         status:'Reserved',  rentalYield:'4.10%', tags:['HIGH CAPITAL GROWTH'],        date:'14 Apr 2026', imgGrad:'from-teal-200 to-teal-300',     lat:-33.75, lng:150.69 },
  { id:'6',  name:'Coastal Sands - 8',     suburb:'Gold Coast',   state:'QLD', price:515000, type:'Land',           status:'Available', rentalYield:'—',     tags:[],                             date:'13 Apr 2026', imgGrad:'from-orange-200 to-orange-300', lat:-28.00, lng:153.43 },
  { id:'7',  name:'Metro Quarter - 14',    suburb:'Adelaide',     state:'SA',  price:680000, type:'Apartment',      status:'Available', rentalYield:'4.50%', tags:['SMSF'],                       date:'12 Apr 2026', imgGrad:'from-indigo-200 to-indigo-300', lat:-34.93, lng:138.60 },
  { id:'8',  name:'Greenfield Hts - 5',    suburb:'Baldivis',     state:'WA',  price:420000, type:'House and Land', status:'Sold',      rentalYield:'3.90%', tags:['HIGH CAPITAL GROWTH'],        date:'11 Apr 2026', imgGrad:'from-green-200 to-green-300',   lat:-32.32, lng:115.84 },
  { id:'9',  name:'Ripley Central - 18',   suburb:'Ripley',       state:'QLD', price:525000, type:'Town House',     status:'Available', rentalYield:'3.65%', tags:['SMSF','HIGH CAPITAL GROWTH'], date:'10 Apr 2026', imgGrad:'from-rose-200 to-rose-300',     lat:-27.67, lng:152.77 },
  { id:'10', name:'Springvale Rise - 44',  suburb:'Cranbourne',   state:'VIC', price:610000, type:'House and Land', status:'Available', rentalYield:'3.55%', tags:['HIGH CAPITAL GROWTH'],        date:'09 Apr 2026', imgGrad:'from-purple-200 to-purple-300', lat:-38.10, lng:145.28 },
]
...
const TAG_STYLES: Record<string, string> = {
  'HIGH CAPITAL GROWTH': 'bg-orange-500 text-white',
  'SMSF':                'bg-green-500 text-white',
}
...
const STATUS_STYLES: Record<string, string> = {
  ...
  'Sold':      'bg-red-50 text-red-600 ring-1 ring-red-200',
}
  ...
  return (
    <div className="relative w-full h-full bg-gradient-to-br from-slate-50 via-teal-50/40 to-sky-50 overflow-hidden rounded-none">
          ...
          >
            <div className={`absolute bottom-full left-1/2 -translate-x-1/2 mb-2.5 z-30 pointer-events-none transition-all duration-150 ${isHovered ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-1'}`}>
              <div className="bg-gray-900/95 backdrop-blur-sm text-white text-[10px] font-medium px-2.5 py-1.5 rounded-lg whitespace-nowrap shadow-xl border border-white/10">
                ...
                <p className="text-gray-300 text-[9px] mt-0.5">{lot.suburb}, {lot.state} · ${lot.price.toLocaleString()}</p>
              </div>
      ...
      <div className="absolute bottom-4 left-4 bg-white/90 backdrop-blur-sm border border-gray-200 rounded-lg px-3 py-1.5 shadow-sm">
        <p className="text-[10px] font-semibold text-gray-600">{lots.length} pins shown</p>
      </div>
...
function LotCard({ lot, selected, onSelect }: { lot: Lot; selected: boolean; onSelect: () => void }) {
  return (
    <div
      onClick={onSelect}
    ...
    >
      {selected && <div className="absolute left-0 top-0 bottom-0 w-0.5 bg-orange-500 rounded-l-xl" />}

      {/* Thumbnail */}
      <div className={`relative w-[100px] shrink-0 bg-gradient-to-br ${lot.imgGrad}`}>
        <div className="absolute top-1.5 left-1.5 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-150">
          <button onClick={e => e.stopPropagation()} className="w-5 h-5 bg-white/90 rounded-md flex items-center justify-center hover:bg-white shadow-sm transition-colors">
      ...
      <div className="flex-1 px-3 py-2.5 min-w-0">
        ...
        <div className="flex items-start justify-between gap-2 mb-1.5">
          <div className="min-w-0">
            <p className="text-xs font-semibold text-gray-800 truncate leading-tight">{lot.name}</p>
            <p className="text-[10px] text-gray-400 flex items-center gap-0.5 mt-0.5">
              <MapPin className="w-2.5 h-2.5 shrink-0" />{lot.suburb}, {lot.state}
            </p>
          </div>
          <p className="text-xs font-bold text-gray-800 shrink-0 whitespace-nowrap">${lot.price.toLocaleString()}</p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-x-3 gap-y-0.5 mb-1.5">
          <div>
            ...
            <p className="text-[10px] text-gray-700 font-medium truncate">{lot.type}</p>
          </div>
          <div>
            <p className="text-[9px] text-gray-400 uppercase tracking-wide font-medium">Status</p>
            <span className={`inline-block text-[9px] font-semibold px-1.5 py-0.5 rounded-md ${STATUS_STYLES[lot.status]}`}>{lot.status}</span>
          </div>
          <div>
            <p className="text-[9px] text-gray-400 uppercase tracking-wide font-medium">Rental Yield</p>
            <p className="text-[10px] text-gray-700 font-medium">{lot.rentalYield}</p>
          </div>
        </div>
        ...
        {lot.tags.length > 0 && (
          <div className="flex flex-wrap gap-1 mb-1.5">
            {lot.tags.map(tag => (
              <span key={tag} className={`text-[9px] font-bold px-1.5 py-0.5 rounded-md ${TAG_STYLES[tag] ?? 'bg-gray-100 text-gray-600'}`}>
                {tag}
              </span>
            ))}
          </div>
        ...
        <div className="flex items-center justify-between pt-1.5 border-t border-gray-50">
          <div className="flex items-center gap-2.5 text-[9px] text-gray-400">
            ...
            <button onClick={e => e.stopPropagation()} className="flex items-center gap-0.5 text-orange-500 hover:text-orange-600 transition-colors">
              <MapPin className="w-2.5 h-2.5" />Distance
            </button>
          </div>
          <div className="flex gap-1">
            <button onClick={e => e.stopPropagation()} className="flex items-center gap-0.5 px-2 py-0.5 text-[9px] font-medium text-gray-600 bg-gray-100 rounded-md hover:bg-gray-200 transition-colors">
              <Pencil className="w-2 h-2" /> Edit
            </button>
            <button onClick={e => e.stopPropagation()} className="flex items-center gap-0.5 px-2 py-0.5 text-[9px] font-medium text-white bg-orange-500 rounded-md hover:bg-orange-600 transition-colors">
...
export default function LotsPage() {
  ...
  function clearAll() {
    setLocation(''); setRadius(20); setPriceMax(5600000)
    setActiveTags(['Listed']); setSearch('')
  }
  ...
  return (
    <div className="-m-5 flex flex-col h-[calc(100vh-56px)] overflow-hidden bg-[#f5f7fa]">
          ...
          <div className="flex items-end gap-0 flex-wrap">
            ...
            <div className="flex flex-col gap-1.5 pr-4 border-r border-gray-100 mr-4 min-w-[180px]">
              ...
              <div className="flex flex-col gap-1">
                <Toggle enabled={radiusEnabled} onChange={() => setRadiusEnabled(v => !v)} label={`Radius ${radius}`} />
                {radiusEnabled && (
                  <input
            ...
            {/* Price */}
            <div className="flex flex-col gap-1.5 pr-4 border-r border-gray-100 mr-4 min-w-[220px]">
              <label className="flex items-center gap-1.5 text-[10px] font-semibold text-gray-400 uppercase tracking-wider">
                <DollarSign className="w-3 h-3" /> Price Search
              </label>
              <div className="flex flex-col gap-1">
            ...
            <div className="flex items-end gap-2 pb-0.5 ml-auto">
              <button className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-gray-700 bg-white border border-gray-200 rounded-lg hover:border-gray-300 hover:bg-gray-50 transition-colors">
                <SlidersHorizontal className="w-3.5 h-3.5 text-gray-500" /> Advanced Filters
              </button>
              <button onClick={clearAll} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-white bg-orange-500 rounded-lg hover:bg-orange-600 transition-colors">
      ...
      <div className="flex flex-1 min-h-0">
        ...
        {/* Left: list panel */}
        <div className="w-[420px] shrink-0 flex flex-col border-r border-gray-200 bg-[#f5f7fa]">
          ...
          <div className="flex items-center justify-between px-3 py-2 bg-white border-b border-gray-200 shrink-0">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-xs text-gray-400">Results Found</span>
              <span className="text-[10px] text-gray-300 mx-0.5">·</span>
              <span className="text-[10px] text-gray-400">{startResult}–{endResult}</span>
            </div>
            <div className="flex items-center gap-1.5">
              ...
              <div className="flex items-center border border-gray-200 rounded-lg overflow-hidden bg-white">
                <button onClick={() => setView('list')} className={`p-1.5 transition-colors ${view==='list' ? 'bg-orange-500 text-white' : 'text-gray-400 hover:bg-gray-50'}`}>
                  <List className="w-3.5 h-3.5" />
                </button>
                <button onClick={() => setView('grid')} className={`p-1.5 transition-colors ${view==='grid' ? 'bg-orange-500 text-white' : 'text-gray-400 hover:bg-gray-50'}`}>
          ...
          <div className="flex-1 overflow-y-auto px-3 py-3 space-y-2">
            {paged.map(lot => (
              <LotCard
                ...
                onSelect={() => setSelectedId(id => id === lot.id ? null : lot.id)}
              />
            ))}
          </div>
              ...
              p === '...' ? (
                ...
                <button
                  ...
                  className={`w-6 h-6 flex items-center justify-center rounded-md text-[10px] font-medium transition-colors ${
                    page === p ? 'bg-orange-500 text-white shadow-sm' : 'text-gray-500 hover:bg-gray-100'
                  }`}
                >
                  {p}
                </button>
